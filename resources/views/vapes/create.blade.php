...
@section('content')
    <form class="me-5 ms-3" action="{{route('createoffer')}}" enctype="multipart/form-data" method="post">
        ...
        <div class="mb-3">
            ...
            <input value="{{old('contacts')}}" type="text" class="form-control @error('contacts') is-invalid @enderror"
                   name="contacts" placeholder="название">
            @error('contacts')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input value="{{old('price')}}" type="number" class="form-control @error('price') is-invalid @enderror"
        ...
        @endforeach
        <div class="form-group">
            <label for="image">Картинка для объявления:</label>
            <br>
