...
@section('content')
    <div class="row">
        <div class="col-md-3">
            <p class="mb-0"><image width="300px" src="{{asset('/storage/images/vapes/' . $vape->image)}}"></image></p>
        </div>
        <div class="col-md-4 ms-2">
            ...
            @foreach($vape->categories as $category)
                <h5 class="mt-5">Категории: {{ $category->name}}</h5>
            @endforeach
            @foreach($vape->brands as $brand)
                <h5>Производитель: {{ $brand->name}}</h5>
            @endforeach
        </div>
        <div class="col-md-4">
